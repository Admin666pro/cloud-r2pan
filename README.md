...
| 绑定名称    | 类型           | 资源名称            | 作用                |
| ------- | ------------ | --------------- | ----------------- |
| `r2`    | R2 Bucket    | `crystal-drive` | 存储上传的文件对象         |
...
- 管理后台：`https://crystal-drive.<你的账号>.workers.dev/admin`（用 `admin` 登录）
...
- **方案 B（可选）**：若想完全自动化，改用一个独立 job 内联设置（注意：secret 一旦已有则保留旧值，幂等）：
  ...
  - name: Ensure admin secret
    ...
    run: |
      echo "${{ secrets.admin }}" | npx wrangler secret put admin --name crystal-drive
  ```
...
- **绑定名与资源名**：绑定名（`r2` / `DB`）是代码里用的名字；资源名同取 `crystal-drive`。
